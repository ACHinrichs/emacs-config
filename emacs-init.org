...
#+begin_src emacs-lisp :tangle yes
  ...
  (load-file (locate-user-emacs-file "ox-latex/bkclasses.el"))
#+end_src
...
#+RESULTS:
| a | Allgemein | plain | (file ~/sciebo/org-roam/templates/template.org) | :target | (file+head %<%Y%m%d%H%M%S>-${slug}.org #+Title: ${title} |
...
# deactivated by removing emacs-lisp language hint and :tanlge yes
#+begin_src 
...
3. Add visual-fill-columns mode
4. Enable visual-fill-column-center-text
5. Add adaptive-wrap-prefix mode
...
#+RESULTS:
| my-text-mode-setup | mixed-pitch-mode | flyspell-mode | copilot-mode | turn-off-auto-fill | text-mode-hook-identify |
